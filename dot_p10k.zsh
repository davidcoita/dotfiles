...
# Left: where am I, and what's git doing.
typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
  dir
  vcs
  newline
...
# Right: only what's relevant to this stack. Cloud/k8s/terraform segments are
# gated by SHOW_ON_COMMAND below, so they stay hidden until you actually use them.
typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
  status
...
# Node: show it in any directory that is actually a JS project. Because fnm now
# honours .nvmrc, this segment doubles as a check that the right version is active.
typeset -g POWERLEVEL9K_NODE_VERSION_PROJECT_ONLY=true
typeset -g POWERLEVEL9K_NODE_VERSION_FOREGROUND=70

# PHP: only inside a PHP project (composer.json / *.php).
typeset -g POWERLEVEL9K_PHP_VERSION_PROJECT_ONLY=true
typeset -g POWERLEVEL9K_PHP_VERSION_FOREGROUND=99

# Kubernetes: GKE contexts render as gke_<project>_<zone>_<cluster>, which is
# 49 characters here and would dominate the prompt. Show cluster + namespace only.
typeset -g POWERLEVEL9K_KUBECONTEXT_SHOW_ON_COMMAND='kubectl|helm|kubens|kubectx|k9s|stern|kubeseal|skaffold|flux|helmfile'
typeset -g POWERLEVEL9K_KUBECONTEXT_CLASSES=(
  '*'  DEFAULT
)
typeset -g POWERLEVEL9K_KUBECONTEXT_DEFAULT_FOREGROUND=134
# gke_project_zone_cluster -> cluster
typeset -g POWERLEVEL9K_KUBECONTEXT_DEFAULT_CONTENT_EXPANSION='${${${P9K_CONTENT##*_}:-$P9K_CONTENT}}${P9K_KUBECONTEXT_NAMESPACE:+/$P9K_KUBECONTEXT_NAMESPACE}'

# Terraform: workspace, but not the noisy "default" one.
typeset -g POWERLEVEL9K_TERRAFORM_SHOW_ON_COMMAND='terraform|tofu|terragrunt'
typeset -g POWERLEVEL9K_TERRAFORM_OTHER_FOREGROUND=38

# gcloud: project only, not the full account string.
typeset -g POWERLEVEL9K_GCLOUD_SHOW_ON_COMMAND='gcloud|gcs|gsutil'
typeset -g POWERLEVEL9K_GCLOUD_PARTIAL_CONTENT_EXPANSION='${P9K_GCLOUD_PROJECT_ID//\%/%%}'

# Only show user@host when it isn't just you on this machine (i.e. over SSH).
typeset -g POWERLEVEL9K_CONTEXT_TEMPLATE='%n@%m'
typeset -g POWERLEVEL9K_CONTEXT_{DEFAULT,SUDO}_{CONTENT,VISUAL_IDENTIFIER}_EXPANSION=
...
# Directory: keep the tail readable in deep monorepos.
typeset -g POWERLEVEL9K_SHORTEN_STRATEGY=truncate_to_unique
typeset -g POWERLEVEL9K_DIR_MAX_LENGTH=40

# Transient prompt: past commands collapse to a bare prompt char, so scrollback
# stays readable instead of repeating the full status line on every entry.
